...
//OPENING
console.log(
  boxen(chalk.blue("Made with ") + chalk.red("❤") + chalk.blue(" by yz"), {
    borderColor: "red",
    padding: 3,
  })
);
...
async function getOutputPath() {
  ...
  if (output_path === 0) {
    outputPath = process.cwd() + "/nft_image/";
  } else {
    const { file_location } = await inquirer.prompt([
      {
        type: "input",
...
async function setWeights(trait) {
  ...
  files.forEach((file, i) => {
    ...
    /**
     * took image file name number and ignore zero before number.
     * @string
     * file > imageName-001.png
     */

    // const getDotIndex = file.lastIndexOf(".");
    // const getLastDashIndex = file.lastIndexOf("-") + 1;
    // const getLoopNumber = fle.substring(getLastDashIndex, getDotIndex);
    // const num = parseInt(getLoopNumber, 10);

    // weights[file] = selectedWeights[names[file] + "_weight"];
    // console.log(`files::: `, file);
  });
  config.weights = weights;
  console.log(`config::: `, config.weights)
}

//ASYNC FOREACH
async function asyncForEach(array, callback) {
  for (let index = 0; index < array.length; index++) {
    // create custom forEach function
...
async function generateImages() {
  ...
  if (config.deleteDuplicates) {
    while (weightedTraits[0].length > 0 && noMoreMatches < 20000) {
      let picked = [];
      console.log("inside loop of deleteDuplicates")
      order.forEach((id) => {
...
async function getFilesForTrait(trait) {
  /**
   * await readdir(basePath + "/" + trait) return [ "ImageName.png" ]
   */
  const files = (await readdir(basePath + "/" + trait)).filter(file => file !== ".DS_Store");
  return files;
